...
> 基于 **Qt6 + DLL 动态链接库** 架构的 TF-IDF 搜索引擎课程设计项目
...
2. **大小写统一** — 全部转为小写
3. **分词** — 按空格、制表符、换行符分割
4. **过滤停用词** — 去除 a, an, the, or, and 等常见停用词
5. **构建倒排索引** — 建立 词 → (文档ID, 词频) 的映射
...
| 成员 | 负责模块 | 主要贡献 |
|------|----------|----------|
| 丘雯杰 | 模块1 + GUI 整合 | 文本文件读取与预处理、GUI 界面设计与实现、DLL 接口封装 |
| 肖晨曦 | 模块2 | 词频统计与倒排索引构建 |
...
> **GitHub 仓库**：[https://github.com/Teriri799/project-group8.git](https://github.com/Teriri799/project-group8.git)
